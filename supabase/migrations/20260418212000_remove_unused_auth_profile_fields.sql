...
drop function if exists roamingreg.roaming_verify_lookup_auth(text, text, text);
drop function if exists roamingreg.roaming_upsert_lookup_auth(jsonb);

drop index if exists roamingreg.idx_coupon_registrations_auth_profile_id;

alter table if exists roamingreg.coupon_registrations
  drop constraint if exists coupon_registrations_auth_profile_id_fkey;

alter table if exists roamingreg.coupon_registrations
  drop column if exists auth_profile_id,
  drop column if exists auth_method;

drop table if exists roamingreg.lookup_auth_profiles;
