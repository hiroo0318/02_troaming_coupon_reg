...
const APIHUB_URL =
  ...
  "https://ifcoupon.sktcoupon.co.kr/gateway/apihubCspSend.api";
...
async function decryptAesBase64(value: string | null) {
  if (!value || !SERVICE_AES_KEY) return value;

  try {
    const iv = SERVICE_AES_IV ? keyToBytes(SERVICE_AES_IV) : new Uint8Array(16);
    const cryptoKey = await crypto.subtle.importKey(
      "raw",
      keyToBytes(SERVICE_AES_KEY),
      { name: "AES-CBC" },
      false,
      ["decrypt"],
    );
...
async function fetchServiceInfo(phoneNumber: string) {
  ...
  const prodList = await decryptAesBase64(prodListRaw);
  const birthDateValue = await decryptAesBase64(birthRaw);
  ...
  return {
    ...
    birthDateValue,
    birthDate,
    isSktSubscriber: !noLineFound,
    isAdult: noLineFound ? null : isAdult(birthDate),
...
async function handleRegister(payload: JsonMap) {
  ...
  try {
    ...
    const serviceCheckedRegistration = await updateRegistration(Number(registration.id), {
      ...
      service_check_code: serviceInfo.header.responseCode,
    ...
    let updatedRegistration = await updateRegistration(Number(registration.id), {
      registration_request: { couponNumber },
    ...
    const subscriptionRequired = Boolean(productCode && category !== "금액권");
